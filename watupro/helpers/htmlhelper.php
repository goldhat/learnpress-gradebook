...
function watupro_paginate_header($exam, $qct, $num_pages, $mode = 'show') {
   ...
   $advanced_settings = unserialize(stripslashes($exam->advanced_settings));
	...
	if($exam->single_page == WATUPRO_PAGINATE_CUSTOM_NUMBER and (($qct % $exam->custom_per_page) == 0) ) {
   	 ...
   	 // adding or !isset here for compatibility with the previous default setting
   	 if(!empty($advanced_settings['show_xofy']) or !isset($advanced_settings['show_xofy'])) $output .= "<p>".sprintf(__('Page %d of %d', 'watupro'), $page_count, $num_pages)."</p>";   	 
   	 $page_count++;   	 
   }
...
function watupro_exam_url_raw($exam_id) {
	global $wpdb;
...
function watupro_screen_reader_text($class) {
	...
	return '<span class="watupro-screen-reader">' . $text . '</span>';
}
