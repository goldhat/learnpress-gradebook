...
class WatuPRO {
    ...
    function add_taking($exam_id, $in_progress=0) {
        ...
        if(empty($taking_id) and !empty($_SESSION['watupro_taking_id_'.$exam_id])) $taking_id = $_SESSION['watupro_taking_id_' . $exam_id];	
     		...
     		if(!$in_progress and !empty($taking_id)) {
     			...
     			$_SESSION['watupro_taking_id_'.$exam_id] = 0;
     		}	
        ...
        if(empty($taking_id)) {
		      ...
		      array('%d','%d','%s','%s','%s','%s','%s','%s','%s','%d','%d', '%s', '%s', '%s'));
        	 ...
        	 if(empty($taking->serialized_questions) and !empty($_POST['watupro_questions'])) {
        	 	$wpdb->query($wpdb->prepare("UPDATE ".WATUPRO_TAKEN_EXAMS." 
        	 		SET serialized_questions=%s, timer_log=%s WHERE ID=%d", $_POST['watupro_questions'], $timer_log, $taking_id));
        	 }
        }
    ...
    function update_taking($taking_id, $points, $grade, $details="", $percent = 0, $grade_obj = null, $catgrades = '', $contact_data = '', $pointspercent = 0, $catgrades_array = array(), $data = null) {
        ...
        $wpdb->query($wpdb->prepare("UPDATE ".WATUPRO_TAKEN_EXAMS." SET 
            ...
            num_empty=%d, max_points=%f, auto_submitted=%d, timer_log=%s WHERE ID=%d", 
			      ...
			      $auto_submitted, $timer_log, $taking_id));
    }  
    	   ...
    	   $sender = watupro_admin_email();
			...
			if($exam->email_admin) {				
				...
				if(empty($dont_email_admin)) {					
					...
					if(!empty($user_email)) $output = sprintf(__("Details of %s:", 'watupro'), $user_data) . "<br><br>".$output;			
	...
	function can_retake($exam) {
		...
		if(!empty($advanced_settings['takings_by_email']) and !empty($_POST['taker_email'])) {
			...
			$num_taken = $wpdb->get_var($wpdb->prepare("SELECT COUNT(ID) FROM ".WATUPRO_TAKEN_EXAMS."
				WHERE exam_id=%d AND email=%s AND in_progress=0", $exam->ID, $_POST['taker_email']));
				
			if($advanced_settings['takings_by_email'] <= $num_taken) {
				echo $descr;
         ...
         // now select number of takings			
			...
			if(!empty($advanced_settings['retake_days_limit'])) {
				...
				$first_date = $wpdb->get_var($wpdb->prepare("SELECT date FROM " . WATUPRO_TAKEN_EXAMS." 
					WHERE user_id=%d AND exam_id=%d AND in_progress=0 ORDER BY ID LIMIT 1", $user_ID, $exam->ID));
				if(!empty($first_date)) {
	...
	function verify_time_limit($exam, $in_progress = null) {
		...
		if(is_user_logged_in() and !$in_progress) {
			...
			if(empty($user_meta_start)) {
				if(!empty($_SESSION['start_time'.$exam->ID])) {
					$_POST['timer_log'] .= "Getting time from session.";
					$start = $_SESSION['start_time'.$exam->ID];
				}
				else {
    ...
    function store_details($exam_id, $taking_id, $question_id, $answer, $points=0, $question_text="", $is_correct=0, $snapshot = '') {
        ...
        if(empty($detail->ID)) {
    		   $wpdb->insert(WATUPRO_STUDENT_ANSWERS,
    			...
    			array("%d","%d","%d","%d","%s","%f","%s", "%d", "%s", "%s", "%d", "%d", "%s", "%d","%s"));    
            ...
            $wpdb->query($wpdb->prepare("UPDATE ".WATUPRO_STUDENT_ANSWERS." SET
               ...
               intval(@$_POST['current_question']), @$_POST['feedback-'.$question_id], intval(@$_POST['question_rating_'.$question_id]), 
               $freetext_answer, $detail->ID ));
            $detail_id = $detail->ID;                 
        } 
    ...
    static function can_access($exam) { 
		...
		// again Namaste but this time no access due to lesson restriction
		 ...
		 if(!empty($advanced_settings['restrict_by_user']) and !empty($advanced_settings['allowed_users'])) {
		 	 ...
		 	 if(!in_array($user_info->data->user_login, $allowed_users) and !in_array($user_info->data->user_email, $allowed_users)) {
...
function watupro_taking_details($noexit = false) {
		...
		tQ.is_survey as is_survey, tQ.calculate_whole as calculate_whole
...
function watupro_initialize_timer() {
   ...
   $exam = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".WATUPRO_EXAMS." WHERE ID=%d", intval($_POST['exam_id'])));
	...
	if(is_user_logged_in()) {
		...
		$_POST['start_time'] = $_SESSION['start_time'.$_POST['exam_id']] = $time;
		$_POST['timer_log'] = $timer_log;
		$taking_id = $_watu->add_taking($_POST['exam_id'],1);
	}
	else {
		 // not logged in		
		 if(empty($_SESSION['start_time'.$_POST['exam_id']])) $_SESSION['start_time'.$_POST['exam_id']] = $time;
		 else $time = $_SESSION['start_time'.$_POST['exam_id']];
	} 
