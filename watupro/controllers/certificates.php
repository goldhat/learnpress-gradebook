...
function watupro_view_certificate() {
	...
	if(empty($_GET['taking_id'])) {
	   ...
	   $replace_fields = array('company' => $taking->field_company, 'phone' => $taking->field_phone, 'field1' => $taking->custom_field1, 'field2' => $taking->custom_field2);	   
	   $exam = $wpdb->get_row($wpdb->prepare("SELECT * FROM " . WATUPRO_EXAMS . " WHERE ID=%d ", $taking->exam_id));
		...
		if(!empty($catgrades_array) and is_array($catgrades_array)) {
			foreach($catgrades_array as $cnt => $catgrade) {	
				...
				$output =  str_replace('%%CATEGORY-NAME-'.$catgrade['cat_id'].'%%', $catgrade['name'], $output);
				...
				$output =  str_replace('%%CATEGORY-PERCENTAGEOFMAX-'.$catgrade['cat_id'].'%%', $percent_of_max, $output);
...
function watupro_taking_pdf() {
	...
	if($taking_id != $_SESSION['watupro_taking_id']) {
