...
function watupro_init() {
   ...
   add_shortcode( 'watupro-myexams', 'watupro_myexams_code' );
	...
	if($db_version < 5.42) watupro_activate(true);
...
function watupro_activate($update = false) {
   ...
   $wpdb->show_errors();
      ...
      watupro_add_db_fields(array(      		
					...
					array("name"=>"custom_field1", "type"=>"VARCHAR(255) NOT NULL DEFAULT ''"),
					array("name"=>"custom_field2", "type"=>"VARCHAR(255) NOT NULL DEFAULT ''"),
		 ...
		 watupro_add_db_fields(array(   		
      	...
      	array("name"=>"namaste_courses", "type"=>"VARCHAR(255) NOT NULL DEFAULT ''"), /* string like |id1|id2| when the quiz is limited to namaste course members */
      	...
      	array("name"=>"thumb", "type"=>"VARCHAR(255) NOT NULL DEFAULT ''"),  
		), WATUPRO_EXAMS);	
      ...
      update_option("watupro_db_version", 5.42);
}
...
function watupro_add_menu_links() {
	...
	if(watupro_intel() and !current_user_can('manage_options')) {
    	...
    	$role_settings = @$role_settings[$roles[0]];
	}
    ...
    add_submenu_page('watupro_exams', ucfirst(WATUPRO_QUIZ_WORD_PLURAL), ucfirst(WATUPRO_QUIZ_WORD_PLURAL), WATUPRO_MANAGE_CAPS, "watupro_exams", 'watupro_exams');
	 ...
	 add_submenu_page( 'watupro_exams' , sprintf(__('All %s Submissions', 'watupro'), ucfirst(WATUPRO_QUIZ_WORD_PLURAL)), sprintf(__('All %s Submissions', 'watupro'), ucfirst(WATUPRO_QUIZ_WORD_PLURAL)), $alltest_caps, "watupro_takings", "watupro_takings"); 
