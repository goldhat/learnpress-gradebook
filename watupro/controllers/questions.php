...
function watupro_questions() {
	...
	if(isset($_POST['ok']) and check_admin_referer('watupro_question')) {
		// add new category?
	...
	if(!empty($_POST['mass_delete']) and check_admin_referer('watupro_questions')) {
	...
	if((!empty($_POST['mass_activate']) or !empty($_POST['mass_deactivate'])) and check_admin_referer('watupro_questions') ) {
		$qids = is_array($_POST['qids']) ? watupro_int_array($_POST['qids']) : array(0);
		$qid_sql = implode(", ", $qids);
	...
	if(!empty($_POST['mass_change_category']) and check_admin_referer('watupro_questions')) {
		$qids = is_array($_POST['qids']) ?  watupro_int_array($_POST['qids']) : array(0);
		$qid_sql = implode(", ", $qids);
	...
	if(!empty($_POST['mass_update']) and check_admin_referer('watupro_questions')) {
		$qids = is_array($_POST['qids']) ?  watupro_int_array($_POST['qids']) : array(0);
		$qid_sql = implode(", ", $qids);
