...
function watupro_export_questions() {
	...
	if(empty($_GET['copy'])) {
		...
		if(empty($GET['legacy'])) {
			...
			.$delim."Difficulty level".$delim."Penalty for non-answering".$delim."Multiple gaps as drop-downs".$delim."Answer columns".$delim."Don't randomize answers";
		}		
	...
	foreach($questions as $question) {
		...
		if(empty($_GET['legacy'])) {
			$row .= $delim.$question['open_end_mode'].$delim.$quote.$question['tags'].$quote.$delim.$question['open_end_display'].
						...
						$delim.$question['num_columns'].$delim.$question['dont_randomize_answers'];
		}	
