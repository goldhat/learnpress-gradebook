...
global $user_email, $user_identity, $question_catids, $post, $do_redirect, $achieved, $percent, $user_ID;	
...
$_POST['watupro_current_taking_id'] = $GLOBALS['watupro_taking_id'] = $_SESSION['watupro_taking_id'] = $taking_id;  // needed in personality quizzes and shortcodes
...
$result = $unresolved_questions = $current_text = $paginated_result = $short_answers = '';
...
foreach ($all_question as $qct=>$ques) {	
      ...
      if($is_empty and empty($ques->is_survey)) $num_empty++;
...
// finally, $do_redirect could be overriden by session
if(!empty($_SESSION['watupro_goto_url'])) {
	$do_redirect = sanitize_text_field($_SESSION['watupro_goto_url']);
	unset($_SESSION['watupro_goto_url']);
}
...
'%%ANSWERS%%', '%%CATGRADES%%', '%%DATE%%', '%%EMAIL%%', '%%MAX-POINTS%%', '%%watupro-share-url%%',
	...
	'%%WRONG_ANSWERS%%', '%%SHORT_ANSWERS%%', '%%CERTIFICATE_ID%%', '%%EMPTY%%', '%%ATTEMPTED%%');
$with_these = array($score, $total,  $percent, $rating, $score, stripslashes($exam->name), wpautop(stripslashes($exam->description)), $achieved,  $certificate, 
   ...
   $max_points,  $share_url, $time_spent, stripslashes($user_name), $avg_points, $avg_percent, $contact_data, $better_than, $pointspercent, $paginated_result, 
   round($achieved), $taken_start_time, $taken_end_time, $num_wrong, $num_wrong, $short_answers, @implode(', ', $certificate_id), $num_empty, ($total - $num_empty));
...
if(empty($advanced_settings['dont_store_taking']) or is_user_logged_in()) {
