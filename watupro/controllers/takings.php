...
function watupro_takings($in_shortcode = false, $atts = null) {	
	...
	WHERE $exam_sql tT.in_progress=$in_progress $filter_sql $class_manager_sql
	...
	$filters_url="dn=".esc_attr(@$_GET['dn'])."&dnf=".esc_attr(@$_GET['dnf'])."&email=".esc_attr(@$_GET['email'])."&emailf=".
		esc_attr(@$_GET['emailf'])."&companyf=".esc_attr(@$_GET['companyf'])."&field_company=".esc_attr(@$_GET['field_company']).
		"&phonef=".esc_attr(@$_GET['phonef'])."&field_phone=".@$_GET['field_phone']."&ip=".@$_GET['ip']."&ipf=".@$_GET['ipf']."&date=".@$_GET['date'].
		"&date2=".@$_GET['date2']."&datef=".@$_GET['datef']."&points=".@$_GET['points']."&pointsf=".@$_GET['pointsf'].
		"&grade=".@$_GET['grade']."&role=".@$_GET['role']."&ugroup=".@$_GET['ugroup']."&quiz_cat_id=".$_GET['quiz_cat_id'].
		"&percent_correct=".@$_GET['percent_correct']."&percentf=".@$_GET['percentf']."&source_url=".@$_GET['source_url']."&in_progress=".$in_progress."&per_page=".$per_page;	
...
function watupro_questions_feedback() {
	...
	$feedbacks = $wpdb->get_results("SELECT SQL_CALC_FOUND_ROWS tA.answer as answer, tA.feedback as feedback, tA.is_correct as is_correct, 
		...
		tQ.question as question , tE.name as quiz_name
		FROM ".WATUPRO_STUDENT_ANSWERS." tA 
		JOIN ".WATUPRO_TAKEN_EXAMS." tT ON tA.taking_id = tT.ID $exam_sql AND tT.in_progress=0
		JOIN ".WATUPRO_QUESTIONS." tQ ON tA.question_id = tQ.ID
		JOIN ".WATUPRO_EXAMS." tE ON tE.ID = tT.exam_id
		WHERE tA.feedback != '' 
		ORDER BY tA.ID DESC LIMIT $offset, $limit");
	$count = $wpdb->get_var("SELECT FOUND_ROWS()");	
