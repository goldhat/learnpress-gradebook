...
function watupro_shortcode( $attr ) {
	...
	if(is_user_logged_in() and empty($advanced_settings['dont_load_inprogress'])) {
...
function watupro_listcode($attr, $content = '') {
	...
	$content = WTPExam::show_list($cat_id, $orderby, $show_status, $content);
...
function watupro_result($atts) {
	...
	if(empty($user_id) and empty($_SESSION['watupro_taking_id'])) return __('N/a', 'watupro');
	
	$quiz_sql = $taking_id_sql = '';
	if(!empty($quiz_id)) $quiz_sql = $wpdb->prepare(" tT.exam_id=%d AND ", $quiz_id);
	if(!empty($_SESSION['watupro_taking_id']) and empty($quiz_id)) $taking_id_sql = $wpdb->prepare(" tT.ID = %d AND ", $_SESSION['watupro_taking_id']);	
...
function watupro_shortcode_paginator($atts) {
	...
	if(is_user_logged_in()) {	
		...
		if(!empty($_SESSION['watupro_taking_id_' . $exam_id])) {
			$in_progress = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".WATUPRO_TAKEN_EXAMS." 
				WHERE ID=%d", $_SESSION['watupro_taking_id_' . $exam_id]));
		}
