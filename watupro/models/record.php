...
class WTPRecord {
	 function export($takings, $exam = null) {
			...
			if(empty($_GET['details']) or empty($exam->ID)) {
				...
				$delim.__('Email', 'watupro').$delim.__('IP', 'watupro'). $contact_field_columns . $delim.__('Date', 'watupro').$delim.__('Points', 'watupro').
				...
				$questions = $wpdb->get_results("SELECT tQ.*, tC.exclude_from_reports 
					...
					$titlerow =__('First name', 'watupro').$delim.__('Last name', 'watupro').$delim.__('Username', 'watupro').$delim.__('Email', 'watupro').$delim
						.__('IP', 'watupro'). $contact_field_columns . $delim.__('Date', 'watupro') . $delim;
					...
					$titlerow .= __('Points', 'watupro').$delim.__('Percent correct', 'watupro').$delim.__('Correct answers', 'watupro').$delim.__('Wrong answers', 'watupro').
			...
			foreach($takings as $taking) {
				   ...
				   $quote . (!empty($taking->last_name) ? $taking->last_name : "N/A") . $quote .  $delim .
					...
					$row .= $delim. $quote . date(get_option('date_format'), strtotime($taking->date)). $quote . $delim;
			  ...
			  if(!empty($_GET['details'])) {
				...
				$row .=	$taking->points.$delim . $taking->percent_correct.$delim .round($taking->percent_of_max*100).$delim. $taking->num_correct. 
	...
	static function export_my_exams($my_exams, $takings, $num_taken) {
		...
		foreach($my_exams as $exam) {
			$final_exams[] = $exam;
			if($num_takings > 1 and empty($disallow_results)) {
