...
class WTPExam {
	   ...
	   if(empty($exam->ID)) throw new Exception(__("Invalid exam ID", 'watupro'));
		...
		foreach($questions as $question) {
			...
			foreach($question['choices'] as $choice) {
				...
				$wpdb->query($wpdb->prepare("INSERT INTO ".WATUPRO_ANSWERS." (question_id,answer,correct,point, sort_order, grade_id, chk_group, is_checked)
					VALUES(%d, %s, %s, %s, %d, %s, %d, %d)", 
					$new_question_id, $choice->answer, $choice->correct, $choice->point, $choice->sort_order, $choice->grade_id, $choice->chk_group, $choice->is_checked));
			}  // end foreach choice
		}
	}
	...
	static function add($vars) {
		...
		$wpdb->query($wpdb->prepare("INSERT INTO ".WATUPRO_EXAMS." SET			
			...
			is_likert_survey=%d, tags=%s, thumb=%s", 
   ...
   // normalizes and sanitizes vars	
	...
	static function show_list($cat_id = 'ALL', $orderby = "tE.ID", $show_status = false, $passed_content = '') {
		 global $wpdb, $user_ID;
	...
	static function paginator($num_questions, $in_progress = null, $is_vertical = false) {		
		$hide_class = ($num_questions > 10) ? 'watupro-auto-hide' : '';
		$vertical_class = $is_vertical ? 'watupro-paginator-vertical' : '';					
		
		$html = '';
		$html .= "<div class='watupro-paginator-wrap watupro-question-paginator-wrap $vertical_class' style='display:none;'><ul class='watupro-paginator watupro-question-paginator watupro-paginator-custom $hide_class'>";
	...
	static function answers_paginator($num_questions) {		
      ...
      <ul class='watupro-paginator watupro-question-paginator watupro-paginator-custom watupro-answers-paginator $hide_class'>";
	   for($i = 0; $i < $num_questions; $i++) {
	      $j = $i+1;
	      if($j == 1) $activeclass='class="active"';
	      else $activeclass = '';
	...
	static function category_paginator($questions, $exam, $in_progress = null, $is_vertical = false) {
		...
		foreach($questions as $cnt => $question) {
			...
			if(!in_array($question->cat_id, $qcat_ids)) {
				...
				$html .= "<li class='WatuPROCatPaginationCatID".$question->cat_id."' id='exam-".$exam->ID."-WatuPROCatPagination-".$gotopage."' onclick='";
	...
	function maybe_show_description($exam, $inside = false, $cnt_questions) {
		global $user_ID;		
		$description = stripslashes($exam->description);	
		$advanced_settings = unserialize(stripslashes($exam->advanced_settings));
		...
		if(!strstr($description, "{{{button")) $button = false;
		  ...
		  if($exam->time_limit > 0) {
		  	$button = "<button class='watupro-start-quiz' style='$style' onclick=\"WatuPRO.InitializeTimer(".($exam->time_limit*60).", ".$exam->ID.", 1);\">$text</button>";
		  }
			else {
				$button = "<button class='watupro-start-quiz' style='$style' onclick=\"WatuPRO.startButton();\">$text</button>";
			}
