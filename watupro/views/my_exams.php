		...
		<?php if($num_taken):?>
			...
			<?php if(($multiuser_access == 'all' and current_user_can(WATUPRO_MANAGE_CAPS))
				or (get_option('watupro_gdpr') == 1)):?>
					<p>
					<?php if(!$in_shortcode):?>					
						<a href="admin.php?page=my_watupro_exams&export_results=1&noheader=1<?php if(!empty($_GET['user_id'])) echo '&user_id='.$_GET['user_id'];?>"><?php _e('Export this data', 'watupro');?></a>
					<?php endif;?>	
					</p>
			<?php endif;?>	 
			<table class="widefat watupro-table">
