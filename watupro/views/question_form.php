<div class="wrap watupro-wrap">
	...
	function newAnswer() {
		...
		var rtfLink = "<a href=\"#\" onclick=\"jQuery('#rtfWin').wtpCenter();tinymce.get('wtpFloatingRTF').setContent(jQuery('#wtpAnswer"+answer_count+
			"').val());jQuery('#floatingAnswerTransferID').val('wtpAnswer"+answer_count+"');return false;\">Rich Text Editor</a><br />";
		...
		<div class="postbox" id="atdiv">
			...
			<div class="inside" style="padding:8px">
				...
				<div id="maxSelections" style='display:<?php echo (empty($question->ID) or $question->answer_type!='checkbox')?'none':'block'?>;'>
					...
					<p id="flashCards"><input type="checkbox" name="is_flashcard" value="1" <?php if(!empty($question->is_flashcard)) echo 'checked'?> onclick="if(this.checked) { this.form.allow_checkbox_groups.checked = false;}"> <?php printf(__('This question uses flashcards (<a href="%s" target="_blank">flashcard design settings</a>). You need to enter the two sides of each flashcard separated by = sign. The respondent should leave flipped the cards that match correctly and unflip the cards that are wrong. (<a href="%s" target="_blank">Learn more about this</a>)', 'watupro'), 'admin.php?page=watupro_flashcard_design', 'http://blog.calendarscripts.info/flashcards/');?></p>
				</div>
				...
				<div id="questionCorrectCondition" style='display:<?php echo (empty($question->ID) or $question->answer_type=='radio')?'none':'block'?>;'>
					...
					<p><?php printf(__('Maximum points this question will reward to the user regardless of other calculations: %s (enter 0 or leave empty for no maximum)', 'watupro'),
		...
		<div class="postbox" id="answersArea" style='display:<?php echo (empty($question) or ($question->answer_type!='gaps' and $question->answer_type!='sort' and $question->answer_type != 'matrix' and $question->answer_type != 'nmatrix'))?'block':'none';?>'>
			...
			<div class="inside" id="answerAreaInside">	
				...
				<p style="border-bottom:1px dotted #ccc" <?php if($i>2) echo "class='wtp-notruefalse'"?>>
					<a href="#" onclick="jQuery('#rtfWin').wtpCenter();tinymce.get('wtpFloatingRTF').setContent(jQuery('#wtpAnswer<?php echo $i?>').val());jQuery('#floatingAnswerTransferID').val('wtpAnswer<?php echo $i?>');return false;">Rich Text Editor</a><br />					 
	...
	<?php if(empty($question->ID)):?>
	  <input type="submit" name="add_blank" value="<?php _e('Save &amp; Add New (Blank)', 'watupro') ?>" style="font-weight: bold;" class="button-primary" />
	<?php endif;?>
	</p>
...
<div id="rtfWin" style="display:none;background:white;border:1pt solid black;padding:10px;width:60%;min-width:480px;">
    ...
    <p><input type="button" value="<?php _e('Done. Close this.', 'watupro');?>" id="floatingOKBtn" onclick="jQuery('#'+this.form.floating_answer_transfer_id.value).val(tinymce.get('wtpFloatingRTF').getContent());jQuery('#rtfWin').hide();"></p>
