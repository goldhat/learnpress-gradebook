...
<div class="wrap watupro-wrap">
	...
	<?php else:?>
		<h1><?php printf(__('All %s Submissions', 'watupro'), ucfirst(WATUPRO_QUIZ_WORD));?></h1>
	<?php endif;?>	
	...
	<input type="hidden" name="page" value="watupro_takings">
		<div><label><?php echo ucfirst(WATUPRO_QUIZ_WORD)?></label> <select name="exam_id" onchange="wtpPopulateGrades(this.value);">
		...
		</select> <input type="text" name="field_company" value="<?php echo stripslashes(@$_GET['field_company'])?>"></div>
		<?php endif;
		...
		</select> <input type="text" name="field_phone" value="<?php echo stripslashes(@$_GET['field_phone'])?>"></div>
		<?php endif;
		...
		</select> <input type="text" name="field_1" value="<?php echo stripslashes(@$_GET['field_1'])?>"></div>
		<?php endif;
		...
		</select> <input type="text" name="field_2" value="<?php echo stripslashes(@$_GET['field_2'])?>"></div>
		<?php endif;
		...
		<div><label><?php _e('Date Taken', 'watupro')?></label> <select name="datef" onchange="this.value == 'range' ? jQuery('#wtpDate2').show() : jQuery('#wtpDate2').hide();">
			...
			<span id="wtpDate2" style='display:<?php echo (empty($_GET['datef']) or $_GET['datef']!='range') ? 'none' : 'inline';?>'>
				- <input type="text" name="date2" value="<?php echo @$_GET['date2']?>"> <i>YYYY-MM-DD</i>
			</span>
		</div>
	...
	<?php else:?>
		...
		<thead>			
			<tr>
				<?php if(!$in_shortcode or (is_user_logged_in() and (current_user_can('watupro_manage_exams') or current_user_can('manage_options')))):?>	<th><input type="checkbox" onclick="WatuPROSelectAll(this);"></th>
				...
				<?php if($has_catgrades and empty($atts['public'])):?>[<a href="#" onclick="jQuery('.watupro_global_grade').toggle();jQuery('.watupro_cat_grade').toggle();return false;"><?php _e('Toggle per category', 'watupro');?></a>]<?php endif;?>	<?php endif?>
				<?php if(!empty($grade_info)) echo '<br><span style="font-size:small;">'.$grade_info.'</span>';?>
				</th>
				...
				<th><?php _e('Details', 'watupro')?></th>
					<?php if($multiuser_access != 'view' and $multiuser_access != 'group_view' and $multiuser_access != 'view_approve' and $multiuser_access != 'group_view_approve'):?><th><?php _e('Delete', 'watupro')?></th><?php endif;?>
				<?php endif;?>	
			</tr>
		</thead>
		<tbody>
		<?php foreach($takings as $taking):
			...
			<?php if(!$in_shortcode or (is_user_logged_in() and (current_user_can('watupro_manage_exams') or current_user_can('manage_options')))):?><td><input type="checkbox" name="tids[]" value="<?php echo $taking->ID?>" class="tids" onclick="toggleMassDelete();"></td>
			...
			<?php endif;?></td><?php endif; // end if showing grade (shortcode condition);?>
			...
			<?php if(!$in_shortcode or (is_user_logged_in() and (current_user_can('watupro_manage_exams') or current_user_can('manage_options')))):?>						
				...
				<?php if($multiuser_access != 'view' and $multiuser_access != 'group_view' and $multiuser_access != 'view_approve' and $multiuser_access != 'group_view_approve'):?><td><a href="#" onclick="deleteTaking(<?php echo $taking->ID?>);return false;"><?php _e('delete', 'watupro')?></a></td><?php endif;?>
				<?php endif;?>
			</tr>
		<?php endforeach;?>
		</tbody>
