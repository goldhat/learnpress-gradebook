...
<div class="wrap watupro-wrap" style="display:block;float:left;">
	...
	<?php if(empty($_GET['export'])):?>
		...
		<?php if(empty($advanced_settings['show_only_snapshot']) and !$disallow_results):?>
			...
			<?php endif;?>
			&nbsp;<a href="#" onclick="jQuery('#detailsText').css({position: 'relative', top: 10, left: 10});jQuery('#catgradesTable').hide();jQuery('#detailsTable').hide();return false;"><?php _e('Snapshot', 'watupro')?></a>  
	...
	<div id="detailsText" style='background:#EEE;padding:5px;width:90%;<?php if(empty($advanced_settings['show_only_snapshot']) and !$disallow_results and $default_view != 'snapshot'):?>position:absolute;top:-9999px;left:-9999px;<?php endif;?>'>	
	...
	<table align="center" class="widefat watupro-table">
		...
		<tbody>
			<?php foreach($answers as $answer):
				...
				?>
				<tr class="<?php echo $class?> watupro-answer-row-<?php echo $answer_class?> watupro-answer-row"><td><?php echo $answer->question_id?></td><td><?php echo $question_text?></td>
				...
				<?php if(current_user_can(WATUPRO_MANAGE_CAPS) or !empty($advanced_settings['show_result_and_points'])):?>		
					<td><?php echo $answer->points;
					if($answer->calculate_whole) echo '<br>'.__('(calculated for the whole question)', 'watupro');?></td>
