<div class="wrap watupro-wrap">
	...
	<form method="post" onsubmit="return watuproValidateForm(this);">
		<p><label><?php _e('Theme name:', 'watupro');?></label> <?php if(empty($theme->ID)):?><input type="text" size="30" name="name" value="<?php echo empty($theme->ID) ? '' : stripslashes($theme->name);?>"> <?php _e('(Only letters, numbers and spaces)', 'watupro'); else:?><b><?php echo stripslashes($theme->name);?></b><?php endif;?></p>
		<p><?php _e('CSS code of the theme:', 'watupro');?><br>
		<textarea rows="20" cols="100" name="css"><?php echo empty($theme->ID) ? '' : $theme->css?></textarea></p>
		<p>
