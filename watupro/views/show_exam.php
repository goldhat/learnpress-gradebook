...
<form action="" method="post" class="quiz-form" id="quiz-<?php echo $exam_id?>" <?php if($exam->time_limit <= 0 and $button):?>style="display:none;"<?php endif;?> enctype="multipart/form-data" <?php if(!empty($exam->no_ajax)):?>onsubmit="return WatuPRO.submitResult(this)"<?php endif;?>>
<?php
...
if($exam->show_pagination and (empty($advanced_settings['paginator_position']) or @$advanced_settings['paginator_position'] == 'top')) echo WTPExam::paginator($total, @$in_progress);
...
<?php
if($exam->show_pagination and !empty($advanced_settings['paginator_position']) and $advanced_settings['paginator_position'] == 'bottom') echo WTPExam::paginator($total, @$in_progress);
...
echo @$text_captcha_html;?><br />
	...
	<?php if(!empty($ui['use_legacy_buttons_table'])):
		...
		<div class="watupro_buttons flex" id="watuPROButtons<?php echo $exam->ID?>">
		  ...
		  <?php if($exam->live_result and $exam->single_page==WATUPRO_PAGINATE_ONE_PER_PAGE):?><div><input type="button" id="liveResultBtn" value="<?php _e('See Answer', 'watupro')?>" onclick="WatuPRO.liveResult();"></div><?php endif;?>		   
	  ...
	  <?php if(($exam->single_page or $exam->store_progress == 0) and is_user_logged_in() and $exam->enable_save_button):?>
	  	<div><input type="button" name="action" onclick="WatuPRO.saveResult(event)" id="save-button" value="<?php _e('Save', 'watupro') ?>" /></div>
	  <?php endif;?>
		<div><?php if(empty($exam->no_ajax)):?><input type="button" name="action" onclick="WatuPRO.submitResult(event)" id="action-button" value="<?php echo empty($advanced_settings['submit_button_value']) ? _wtpt(__('Submit', 'watupro')) : $advanced_settings['submit_button_value']; ?>" <?php echo $submit_button_style?> />
		<?php else:?>
	...
	<input type="hidden" name="start_timestamp" id="startTimeStamp" value="<?php echo empty($_SESSION['start_time'.$exam->ID]) ? current_time('timestamp') : $_SESSION['start_time'.$exam->ID];?>" />
	...
	<input type="hidden" name="watupro_taker_field1" value="<?php echo @$_POST['watupro_taker_field1']?>">
	<input type="hidden" name="watupro_taker_field2" value="<?php echo @$_POST['watupro_taker_field2']?>">
...
if(!empty($advanced_settings['user_choice'])):?>WatuPRO.userChoice = <?php echo empty($_POST['wtpuc_ok']) ? 0 : 1?>;<?php echo "\n"; endif;
...
WatuPRO.TimerTurnsRed = <?php echo intval(@$advanced_settings['timer_turns_red'])?>;
